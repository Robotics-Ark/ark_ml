#!/usr/bin/env python3
"""
Debug script for Pi05 rollout pipeline - with detailed tracing
"""
...
def debug_pi05_detailed():
    print("Debugging Pi05 rollout pipeline with detailed tracing...")
    ...
    print("\n1. Testing processor separately...")
    try:
        ...
        print(f"  Max instruction token value: {processed['instruction'].max().item()}")
        print(f"  Min instruction token value: {processed['instruction'].min().item()}")
        print(f"  First few instruction tokens: {processed['instruction'][:10].tolist()}")
    ...
    print("\n2. Testing policy with preprocessed tokens directly...")
    try:
        # Use the tokens from the processor
        preprocessed_obs = processor(dummy_obs)
        
        # Test what happens when we call model.predict with this
        print(f"  Model text embedding vocab size: {policy.text_embedding.num_embeddings}")
        action = policy.predict(preprocessed_obs)
        print(f"  Direct policy action shape: {action.shape}, values: {action}")
        print("  ✓ Direct policy test with preprocessed tokens passed")
    except Exception as e:
        print(f"  ✗ Direct policy test with preprocessed tokens failed: {e}")
        import traceback
        traceback.print_exc()
        return False

    print("\n3. Testing full node pipeline...")
